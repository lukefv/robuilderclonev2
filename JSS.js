...
function setCSS(ElementName, Attribute, NewValue) {
    ...
    let Element = ElementName, OriginalStyle = Element.getAttribute("style").split("; "), FoundAttribute = false, i, p, ReapplyStyle = "", CurrentCheck, FirstPart, SecondPart;
    OriginalStyle.pop();
...
function removeCSS(Element, Attribute) {
    ...
    let OriginalStyle = Element.getAttribute("style").split("; ");
    OriginalStyle.pop();
    for (i = 0; i < OriginalStyle.length; i += 1) {
        CurrentCheck = "|| " + OriginalStyle[i] + " ||";
        FirstPart = CurrentCheck.split("|| ").pop().split(": ")[0];
...
function getCSS(ElementName, Attribute) {
    ...
    let Element = ElementName, OriginalStyle = Element.getAttribute("style").split("; "), i, CurrentCheck, FirstPart, SecondPart;
...
function getPageURLPath(Part) {
    if (window.location.protocol == 'file:' || (window.location.origin.includes("https") == false)) {
        return null;
    }
...
;function timestampToString(Seconds) {
    ...
    for (let i = 0; i < ValueKeys.length; i++) {
        let ValueTake = Values[ValueKeys[i]];
...
function OutlineHoverOver(Element) {
    Element.style.backgroundColor = ThemeColors.GreyOutline;
}
function OutlineHoverOut(Element) {
    Element.style.backgroundColor = "rgb(255, 255, 255, 0)";
}
function isInViewport(Elem) {
    let ObjectRect = Elem.getBoundingClientRect();
    return (ObjectRect.y) + (Elem.offsetHeight) > 0 && ObjectRect.y < (window.innerHeight || document.documentElement.clientHeight);
}
function SmoothScrollAnim() {
    let ScrollInElements = document.querySelectorAll('[scrollanim]');
    for (let i = 0; i < ScrollInElements.length; i++) {
        let CurrentElem = ScrollInElements[i];
        setCSS(CurrentElem, "opacity", "0");
        if (isInViewport(CurrentElem) == true) {
            CurrentElem.removeAttribute("scrollanim");
            let OriginalTransition = getCSS(CurrentElem, "transition");
            setCSS(CurrentElem, "transition", "all 1s ease");
            CurrentElem.style.opacity = "1";
            if (OriginalTransition != null) {
                async function RevertProp() {
                    await sleep(1500);
                    setCSS(CurrentElem, "transition", OriginalTransition);
                }
                RevertProp();
            }
        }
